...
def getPipeline(device, height, width, bitrate, format, rotation, framerate, timestamp):
    ...
    if device == "testsrc":
            ...
            s_src = "libcamerasrc ! capsfilter caps=video/x-raw,width={1},height={2},format=NV12{4},colorimetry=bt601,interlace-mode=progressive ! {3} ! v4l2h264enc extra-controls=\"controls,repeat_sequence_header=1,video_bitrate_mode=1,h264_profile=3,video_bitrate={0}\" ! video/x-h264,profile=main,level=(string)4".format(bitrate*1000, width, height, devrotation, framestr)
            pipeline_str = "( {s_src} {ts} ! queue max-size-buffers=1 name=q_enc ! h264parse ! rtph264pay config-interval=1 name=pay0 pt=96 )".format(**locals())
